...
{
    //
    public $owneremail = "raphogochukwu@gmail.com";
...
{
    // Fetch all traders from the database in alphabetical order by name
    $traders = Trader::orderBy('name', 'asc')->get();

    // Pass the sorted list of traders to the traders.blade.php view
    ...
    public function copyTrade(Request $req, $tradeId)
    {
        ...
        if ($voucherAmount > 0) {
            ...
            // Check if the amount is less than the minimum trade value
            if ($amount < $trade->minimum) {
                return back()->with('error', 'Amount must not be less than the minimum trade value of ' . $trade->minimum . '.');
            }

            // Check if the amount is greater than the maximum trade value
            if ($amount > $trade->maximum) {
                return back()->with('error', 'Amount must not exceed the maximum trade value of ' . $trade->maximum . '.');
            }
        }
        ...
        // If not subscribed, create a new subscription for the user !$isSubscribed && 
        if ($user_funds->balance >= $amount) {
            ...
            # code...
            return back()->with('error', 'You have already subsribed to the trade or you need to top up to subscbibe to this trade');
        }
        ...
        if ($voucherAmount > 0) {
            ...
            if ($amount > $plan_from_db->maximum) {
                # code...
                return back()->with('error', "The $$amount investment amount you entered is above the selected plan maximum amount");
            } else {
                if ($amount > $user_funds->balance ) {
                    # code...
                    return back()->with('error', "Account balance of $$user_funds->balance is low for $$amount investment amount, please try a lower amount or fund your account");
                } else {
                    # code...
                    //calaculting profit and dates
                    $raw_profit = $amount * $plan_from_db->percentage;
                    ...
                    $days_string = json_encode($days_array);
                    $new_bal = $user_funds->balance - $amount;
                    $new_trading_balance = $user_funds->currentinvestment + $amount;
                    $saveArray = [
                        'investmentplan' => $plan,
                        'investmentpercent' => $plan_from_db->percentage,
                        'investmentdate' => Carbon::now(),
                    ...
                    if ($result) {
                        ...
                        $user_funds->currentinvestment = $new_trading_balance;
                        $user_funds->balance = $new_bal;
    ...
    'id_document' => 'required|file|mimes:jpg,jpeg,png,pdf|max:991920',
], [
    'id_document.required' => 'Please upload your ID document.',
    'id_document.mimes' => 'The ID document must be a JPG, PNG, or PDF file.',
    'id_document.max' => 'The ID document must not be larger than 80MB.',
]);
        ...
        if ($existingSubmission) {
            ...
            $user = User::where("id", $this->logged_in_user()->id)->first();
            $email = $user->email;
            $mail = "Your Kyc has been updated and is pending verification";
        ...
        $user = User::where("id", $this->logged_in_user()->id)->first();
            $email = $user->email;
            $mail = "Your Kyc verification request has been recieved and is pending verification";
            $mailtitle = "KYC verification";
            $emaildata = ['data' => $email, 'email_body' => $mail, 'email_header' => $mailtitle];
    ...
    public function Tradesinsub(){
       $marketTrades = MarketTrade::with('subscription')->latest()->paginate(10);  // Ensure pagination
    return view('user.market-trades.index', compact('marketTrades')); 
    }

    public function viewTrades($subscriptionId)
    {
        $subscription = TradeSubscription::with(['trade', 'user'])->findOrFail($subscriptionId);
    
        // Paginate the market trades
        $marketTrades = MarketTrade::where('subscription_id', $subscription->id)->paginate(10);
    
        return view('dashb.market-trades.index', compact('subscription', 'marketTrades'));
    }
